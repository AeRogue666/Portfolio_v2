...
const articlePublishedTime = computed(() => formatISO(page.value?.created_at)), // dayjs(page.value?.created_at).locale(locale.value).format()
    articleModifiedTime = computed(() => formatISO(page.value?.updated_at)); // dayjs(page.value?.updated_at).locale(locale.value).format()
...
useHeadSafe(() => ({
    ...
    meta: [
        // Meta names
        { name: 'description', content: t('seo.page.description', { pagetitle: t('breadcrumb.legal_notices') }) },
        // Meta properties
        { property: 'og:title', content: t('seo.page.title', { pagetitle: t('breadcrumb.legal_notices') }) },
        { property: 'og:description', content: t('seo.page.description', { pagetitle: t('breadcrumb.legal_notices') }) },
