...
import type { AccessibilityReportResponse } from '~/server/api/accessibility-report.get.js';
...
const filteredCriteria = computed(() => {
    ...
    return reportData.value.criteria.filter((item: { thematic: string; status: string; criterion: string; id: string; }) => {
        ...
        const matchSearch = !query ||
            item.criterion.toLowerCase().includes(query) ||
...
const stats = computed(() => {
    ...
    const conformes = reportData.value.criteria.filter((c: { status: string; }) => c.status === "C").length;
    const nonConformes = reportData.value.criteria.filter((c: { status: string; }) => c.status === "NC").length;
    const nonApplicables = reportData.value.criteria.filter((c: { status: string; }) => c.status === "NA").length;
    ...
    return {
        ...
        nonConformes,
        nonApplicables,
        conformePercent: total > 0 ? Math.round((conformes / total) * 100) : 0
    };
});
...
const thematicOptions = computed(() => {
    ...
    return [
        ...
        ...reportData.value.thematics.map((t: { label: any; id: any; }) => ({
            label: t.label,
            value: t.id,
        }))
    ];
...
const getThematicColor = (thematicId: string): string => colorMap[thematicId] || 'gray';
...
const getAccessibilityColor = (accessibility: string): string => {
    const score = parseInt(accessibility, 10);
    if (isNaN(score)) return 'bg-gray-500/20';
...
<template>
    <ArticleLayout>
        <template #header>
            ...
            <h1 id="article-title" class="font-bold fs-heading">{{ reportData?.title || t('loading.loading_feed') }}
            </h1>
            <p v-if="reportData" class="text-base text-(--text-2) fs-subtitle">{{ reportData.description }}</p>
        </template>

        <div v-if="pending && !reportData" class="py-12 text-center">
            <p class="fs-body">{{ t('loading.loading_feed') }}...</p>
        </div>

        <div v-else-if="reportData" :class="{ 'opacity-50 pointer-events-none transition-opacity': pending }">
            <!-- Résumé Exécutif -->
            ...
            <section class="mb-12" aria-labelledby="rgaa-heading">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 mb-6 gap-4 overflow-x-auto">
                    <div class="flex flex-col items-baseline">
                        ...
                        <UInput v-model="searchQuery" id="rgaa-search" icon="fa7-solid:magnifying-glass"
                            autocomplete="off" :ui="{
                    ...
                    <div class="flex flex-col items-baseline">
                        ...
                        <USelect v-model="selectedThematic" :items="thematicOptions" id="thematic-select"
                            :aria-label="reportData.thematicFilterLabel" value-key="value" label-key="label"
                            variant="ghost" color="neutral" class="transition-colors fs-body" :ui="{
                    ...
                    <div class="flex flex-col items-baseline">
                        ...
                        <USelect v-model="selectedStatus" :items="statusOptions" id="status-select"
                            :aria-label="reportData.statusFilterLabel" value-key="value" label-key="label"
                            variant="ghost" color="neutral" class="transition-colors fs-body" :ui="{
                                base: 'bg-(--bg-2) min-w-[12rem]',
                                content: 'bg-(--bg-2) min-w-[12rem]',
                                value: grayscale && colorMode.value == 'dark' ? 'text-inverted' : '',
                    ...
                    <table class="w-full fs-body">
                        ...
                        <tbody>
                            <tr v-for="criteria in filteredCriteria" :key="criteria.id"
                                class="border-b border-(--border-subtle) hover:bg-(--bg-2) transition-colors">
                                <td class="px-4 py-3 font-mono fs-body font-semibold text-blue-500"
