...
const { data: postsData, status, execute } = await useFetch('/api/search', {
...
<template>
    <UDrawer v-model:open="isOpen" :title="t('search.title')" :description="t('search.description')" :handle="false"
        ...
        <!-- Trigger -->
        <UButton :aria-label="t('search.aria_label')" :aria-description="t('search.aria_description')" color="neutral"
            variant="subtle" icon="fa7-solid:search" tabindex="0"
